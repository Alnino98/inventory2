...
class Controller_Items extends Admin_Controller 
{
	...
	*/
	public function fetchBrandData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('viewBrand', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-warning btn-sm" onclick="editBrand('.$value['id_barang'].')" data-toggle="modal" data-target="#editBrandModal"><i class="fa fa-pencil"></i></button>';	
			}
			
			if(in_array('deleteBrand', $this->permission)) {
				$buttons .= ' <button type="button" class="btn btn-danger btn-sm" onclick="removeBrand('.$value['id_barang'].')" data-toggle="modal" data-target="#removeBrandModal"><i class="fa fa-trash"></i></button>
				';
			}				
	...
	*/
	public function update($id)
	{
		...
		if($id) {
			$this->form_validation->set_rules('edit_kode_barang', 'Kode Barang', 'trim|required');
			$this->form_validation->set_rules('edit_jenis_barang', 'Jenis Barang', 'trim|required');
			$this->form_validation->set_rules('edit_nama_barang', 'Nama Barang', 'trim|required');
			$this->form_validation->set_rules('edit_banyak_barang', 'Banyak Barang', 'trim|required');
			$this->form_validation->set_rules('edit_tgl_masuk', 'Tanggal Masuk', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
					...
					'jenis_barang'=> $this->input->post('edit_jenis_barang'),
					'nama_barang'=> $this->input->post('edit_nama_barang'),
					'banyak_barang'=>$this->input->post('edit_banyak_barang'),
